...
class GrandPrixMikadoClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_grandprix_core_action_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_grandprix_core_action_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'grandprix-core' );
		
		if ( isset( $_COOKIE[ 'mkdf-like_' . $post->ID ] ) ) {
			$class = 'mkdf-like liked';
			$title = esc_html__( 'You already like this!', 'grandprix-core' );
		}
...
if ( ! function_exists( 'grandprix_core_create_like' ) ) {
	function grandprix_core_create_like() {
		GrandPrixMikadoClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'grandprix_core_create_like' );
}