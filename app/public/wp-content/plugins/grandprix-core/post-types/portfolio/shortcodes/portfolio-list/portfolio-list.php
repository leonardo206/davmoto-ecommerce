...
class PortfolioList implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map( array(
					'name'     => esc_html__( 'Portfolio List', 'grandprix-core' ),
					'base'     => $this->getBase(),
					...
					'params'   => array(
						array(
							'type'        => 'dropdown',
							'param_name'  => 'type',
							'heading'     => esc_html__( 'Portfolio List Template', 'grandprix-core' ),
							'value'       => array(
								esc_html__( 'Gallery', 'grandprix-core' ) => 'gallery',
						...
						array(
							...
							'heading'     => esc_html__( 'Number of Portfolios Per Page', 'grandprix-core' ),
							'description' => esc_html__( 'Set number of items for your portfolio list. Enter -1 to show all.', 'grandprix-core' ),
							'value'       => '-1'
						),
						array(
							'type'        => 'dropdown',
							...
							'description' => esc_html__( 'Set image proportions for your portfolio list.', 'grandprix-core' ),
						...
						array(
							...
							'heading'     => esc_html__( 'Enable Fixed Image Proportions', 'grandprix-core' ),
							'value'       => array_flip( grandprix_mikado_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Set predefined image proportions for your masonry portfolio list. This option will apply image proportions you set in Portfolio Single page - dimensions for masonry option.', 'grandprix-core' ),
						...
						array(
							...
							'value'      => array(
								esc_html__( 'Standard - Move', 'grandprix-core' )                   => 'standard-move',
						...
						array(
							...
							'dependency' => array( 'element' => 'item_style', 'value' => array( 'standard-move','standard-shader', 'standard-switch-images' ) ),
							'group'      => esc_html__( 'Content Layout', 'grandprix-core' )
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'heading'    => esc_html__( 'Pagination Type', 'grandprix-core' ),
							'value'      => array(
								esc_html__( 'None', 'grandprix-core' )            => 'no-pagination',
								esc_html__( 'Standard', 'grandprix-core' )        => 'standard',
								esc_html__( 'Load More', 'grandprix-core' )       => 'load-more',
								esc_html__( 'Infinite Scroll', 'grandprix-core' ) => 'infinite-scroll'
							),
							'group'      => esc_html__( 'Additional Features', 'grandprix-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'load_more_top_margin',
							'heading'    => esc_html__( 'Load More Top Margin (px or %)', 'grandprix-core' ),
						...
						array(
							'type'       => 'dropdown',
							'param_name' => 'filter',
							'heading'    => esc_html__( 'Enable Category Filter', 'grandprix-core' ),
							'value'      => array_flip( grandprix_mikado_get_yes_no_select_array( false ) ),
							'group'      => esc_html__( 'Additional Features', 'grandprix-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Order By', 'grandprix-core' ),
							'value'      => array(
								esc_html__( 'Name', 'grandprix-core' )  => 'name',
								esc_html__( 'Count', 'grandprix-core' ) => 'count',
								esc_html__( 'Id', 'grandprix-core' )    => 'id',
								esc_html__( 'Slug', 'grandprix-core' )  => 'slug'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Filter Text Transform', 'grandprix-core' ),
							'value'      => array_flip( grandprix_mikado_get_text_transform_array( true ) ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'grandprix-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Filter Bottom Margin (px or %)', 'grandprix-core' ),
							'dependency' => array( 'element' => 'filter', 'value' => array( 'yes' ) ),
							'group'      => esc_html__( 'Additional Features', 'grandprix-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Enable Article Animation', 'grandprix-core' ),
							'value'       => array_flip( grandprix_mikado_get_yes_no_select_array( false ) ),
							'description' => esc_html__( 'Enabling this option you will enable appears animation for your portfolio list items', 'grandprix-core' ),
							'group'       => esc_html__( 'Additional Features', 'grandprix-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$additional_params['holder_data']          = grandprix_mikado_get_holder_data_for_cpt( $params, $additional_params );
		...
		$html = grandprix_core_get_cpt_shortcode_module_template_part( 'portfolio', 'portfolio-list', 'portfolio-holder', $params['type'], $params, $additional_params );
		...
		if ( $params['type'] == 'masonry' && $params['enable_fixed_proportions'] === 'yes' ) {
			...
			switch ( $fixed_image_size ) {
				...
				case 'large-width-height':
					$thumb_size = 'grandprix_mikado_image_huge';
					break;
				default :
					$thumb_size = 'full';
					break;
	...
	public function getStandardContentStyles( $params ) {
		...
		if ( ! empty( $margin_top ) ) {
			if ( grandprix_mikado_string_ends_with( $margin_top, '%' ) || grandprix_mikado_string_ends_with( $margin_top, 'px' ) ) {
		...
		if ( ! empty( $margin_bottom ) ) {
			if ( grandprix_mikado_string_ends_with( $margin_bottom, '%' ) || grandprix_mikado_string_ends_with( $margin_bottom, 'px' ) ) {
	...
	public function getLoadMoreStyles( $params ) {
		...
		if ( ! empty( $params['load_more_top_margin'] ) ) {
			...
			if ( grandprix_mikado_string_ends_with( $margin, '%' ) || grandprix_mikado_string_ends_with( $margin, 'px' ) ) {
				$styles[] = 'margin-top: ' . $margin;
			} else {
				$styles[] = 'margin-top: ' . grandprix_mikado_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getFilterHolderStyles( $params ) {
		...
		if ( ! empty( $params['filter_bottom_margin'] ) ) {
			...
			if ( grandprix_mikado_string_ends_with( $margin, '%' ) || grandprix_mikado_string_ends_with( $margin, 'px' ) ) {
				...
				$styles[] = 'margin-bottom: ' . grandprix_mikado_filter_px( $margin ) . 'px';
			}
		}
	...
	public function getItemLink() {
		...
		return apply_filters( 'grandprix_mikado_filter_portfolio_external_link', $portfolio_link );
	}
	...
	public function getItemLinkTarget() {
		...
		return apply_filters( 'grandprix_mikado_filter_portfolio_external_link_target', $portfolio_link_target );
	}
